...
export default function StudentDashboard({ user, logout }) {
  ...
  const stats = [
    { icon: BookOpen, label: 'Active Courses', value: activeCourses.length, color: 'from-blue-500 to-blue-600', testId: 'active-courses-count' },
    { icon: Award, label: 'Certificates', value: certificates.length, color: 'from-blue-600 to-blue-700', testId: 'certificates-count' },
    { icon: TrendingUp, label: 'Completed', value: completedCourses.length, color: 'from-blue-700 to-blue-800', testId: 'completion-rate' },
    { icon: BarChart2, label: 'Avg Progress', value: `${avgProgress}%`, color: 'from-blue-800 to-blue-900', testId: 'avg-progress' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-slate-900" data-testid="dashboard-title">
              Welcome back, <span className="bg-gradient-to-r from-primary to-blue-800 bg-clip-text text-transparent">{user?.name?.split(' ')[0]}</span>!
            </h1>
        ...
        <Tabs defaultValue="active" className="w-full">
          <TabsList className="bg-white border border-slate-200 p-1 rounded-2xl mb-6 w-fit shadow-sm">
            <TabsTrigger value="active" data-testid="active-tab" className="rounded-xl font-semibold text-sm px-5 py-2 data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
              Active Courses
            </TabsTrigger>
            <TabsTrigger value="completed" data-testid="completed-tab" className="rounded-xl font-semibold text-sm px-5 py-2 data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
              Completed
            </TabsTrigger>
            <TabsTrigger value="certificates" data-testid="certificates-tab" className="rounded-xl font-semibold text-sm px-5 py-2 data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
              Certificates
            </TabsTrigger>
            <TabsTrigger value="badges" data-testid="badges-tab" className="rounded-xl font-semibold text-sm px-5 py-2 data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
              🏆 Badges
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="active" data-testid="active-courses">
            {loading ? (
              ...
              <div className="text-center py-20 bg-white rounded-3xl border border-slate-100" data-testid="no-active-courses">
                ...
                <button onClick={() => navigate('/courses')} className="btn-shine px-6 py-3 rounded-2xl font-bold text-white bg-primary shadow-md inline-flex items-center gap-2">
                  Browse Courses <ArrowRight size={16} />
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {activeCourses.map((enrollment) => (
                  <div key={enrollment.id} className="group bg-white rounded-3xl border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-blue-50 hover:-translate-y-0.5 transition-all duration-300" data-testid={`enrollment-${enrollment.id}`}>
                    <div className="flex gap-4 p-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-slate-900 text-sm mb-1 line-clamp-2 group-hover:text-primary transition-colors">{enrollment.course?.title}</h3>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-xs font-bold text-primary" data-testid={`progress-${enrollment.id}`}>{Math.round(enrollment.progress)}%</span>
                          </div>
                    ...
                    <div className="px-5 pb-5">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-blue-50 text-primary font-bold text-sm hover:bg-primary hover:text-white transition-all duration-200"
                      >
                  ...
                  <div key={enrollment.id} className="bg-white rounded-3xl border border-slate-100 overflow-hidden">
                    <div className="flex gap-4 p-5">
                      ...
                      <div className="flex-1">
                        ...
                        <button
                          onClick={() => navigate(`/course/${enrollment.course_id}/learn`)}
                          className="text-sm font-bold text-primary hover:text-blue-800 flex items-center gap-1 transition-colors"
                        >
